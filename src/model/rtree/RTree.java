...
public class RTree {
    ...
    private float[] getSearchingArea() {
        ...
        //In order to make the area more concrete, we divide by a number stablish by our own criteria
    }
    ...
    public ArrayList<RTreeElement> getPointsByProximity(Point centerPoint) {
        ...
        float minX = centerPoint.getX() - Math.abs(centerPoint.getX() - dimensions[0]/2);
        float maxX = dimensions[0]/2 + centerPoint.getX();
        float minY = centerPoint.getY() - Math.abs(centerPoint.getY() - dimensions[1]/2);
